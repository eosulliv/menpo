...
def ears_ibug_55_to_ears_ibug_14(pcloud):
    ...
    ind = np.hstack(([0, 4, 8, 12, 16, 19], [20, 25, 31, 34], [35, 39, 44, 48]))
    new_pcloud = LabelledPointUndirectedGraph.init_from_indices_mapping(
        pcloud.points[ind], all_connectivity, mapping)
...
def ears_ibug_55_to_ears_ibug_55_trimesh(pcloud):
    r"""
...
@labeller_func(group_label='ear_ibug_55_trimesh')
def ears_ibug_14_to_ears_ibug_14_trimesh(pcloud):
    r"""
    Apply the IBUG 14-point semantic labels, with trimesh connectivity.
      - tri
    References
    ----------
    .. [1] http://www.multipie.org/
    ...
    tri_list = np.array([[12, 13,  2], [ 3, 12,  2], [ 1, 13,  0],
                         [10, 13, 12], [13,  7,  2], [ 7,  1,  2],
                         [ 1,  7, 13], [13,  6,  0], [ 6, 10,  0],
                         [10,  6, 13], [ 3,  8, 12], [ 8,  4, 12],
                         [ 4,  8,  3], [11, 10, 12], [11,  5,  0],
                         [10, 11,  0], [ 4,  9, 12], [ 9, 11, 12],
                         [ 9,  4,  5], [11,  9,  5]])
    new_pcloud = TriMesh(pcloud.points, trilist=tri_list)
